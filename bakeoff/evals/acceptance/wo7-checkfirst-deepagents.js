...
(async () => {
  const dbPath = path.join(process.cwd(), 'runs', 'wo7-checkfirst-da.db');
  for (const s of ['', '-wal', '-shm', '.langgraph.db']) fs.rmSync(dbPath + s, { force: true });

  const runId = `wo7-da-${Date.now()}`;
  ...
  const { resumeRun } = await import('../../deepagents/resume.js');
  const { writeTerminal, sqlProbe } = await import('../../deepagents/entry.js');
  ...
  const out = await resumeRun(fixture,
    ...
    report,
    { threadId: `digest-${runId}`, checkpointPath: dbPath + '.langgraph.db',
      interruptId: 'int_irrelevant', presentedArgs: args, argsJson: canonicalJson(args),
  ...
  const checks = [
    ...
    ['harness was never even invoked', () => assert.strictEqual(out.harness_native_resume, undefined)],
  ];
  let pass = 0;
  console.log('Leg 3 (Deep Agents) — check-first SKIPS the send when a record already exists\n');
