'use strict';
// Leg 3 acceptance: the budget-exhausted terminal state. No golden case covers it
...
(async () => {
  ...
  const dbPath = path.join(process.cwd(), 'runs', 'wo5-budget-da.db');
  for (const s of ['', '-wal', '-shm']) fs.rmSync(dbPath + s, { force: true });

  const { run } = await import('../../deepagents/entry.js');
  ...
  let pass = 0;
  console.log('Leg 3 acceptance — budget-exhausted terminal state (DIGEST_STEP_BUDGET=2)\n');
