'use strict';
// WO-5 acceptance: the budget-exhausted terminal state. No golden case covers it
...
(async () => {
  ...
  const dbPath = path.join(process.cwd(), 'runs', 'wo5-budget.db');
  for (const s of ['', '-wal', '-shm']) fs.rmSync(dbPath + s, { force: true });

  const { run } = await import('../../mastra/entry.js');
  ...
  let pass = 0;
  console.log('WO-5 acceptance — budget-exhausted terminal state (DIGEST_STEP_BUDGET=2)\n');
