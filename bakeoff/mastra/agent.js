...
export function createDigestAgent({ model } = {}) {
  return new Agent({
    ...
    defaultOptions: {
      ...
      // WORKAROUND (D-39): Moonshot's Anthropic-compatible endpoint returns a `thinking`
      // content block with NO `signature` field, and @ai-sdk/anthropic's response schema
      // requires content[].signature to be a string -- so every Kimi call fails validation
      // with "Invalid JSON response" even though the HTTP call returned 200 with a correct
      // tool_use. Kimi K2.6 emits thinking on every call, so this is a 100% failure without
      // the workaround. Disabling thinking removes the offending block.
