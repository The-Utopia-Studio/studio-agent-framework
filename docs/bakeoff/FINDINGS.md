...
| Persistence required for resume | LibSQL store (or **ConvexStore — see Y.2c, confirmed native STATE-1a pass with a real SIGKILL and an independent-process read-back**) | **SqliteSaver — docs' `MemorySaver` default is unsafe** | durable adapter in `db.ts` |
...
## Y.2b The Mastra half of Y.1 measured the wrong store

**Recorded 2026-08-25, after review.** [`studio-standard-agent-framework` PR #1](https://github.com/The-Utopia-Studio/studio-standard-agent-framework/pull/1) finding 2 is that the 24-25 Aug Mastra probe "measured the wrong thing" because it used Mastra's **default LibSQL store**. That criticism applies to leg 1 here in full: `mastra/entry.js` constructed `new LibSQLStore({ url: 'file:<db>.mastra.db' })`.

`@mastra/convex@1.5.4` exports a `ConvexStore` implementing the full storage contract, and its exports include `TABLE_WORKFLOW_SNAPSHOT` / `mastraWorkflowSnapshotsTable` -- so **workflow snapshots**, the thing a suspended approval run resumes from, land in Convex tables declared in our own schema. That is exactly the configuration STATE-1a permits: vendor state is fine *if it lands in our schema*.

So Y.1's verdict for Mastra -- "native resume unmet" -- is **true as configured and under-qualified as stated**. Deleting a vendor-local SQLite file is a different test from pointing the vendor at our own authoritative store; with `ConvexStore` there is no vendor-local file to delete, and the question becomes whether resume works when *our* Convex tables are the only state. That has not been measured.

The store is now pluggable (`makeStore()` in `mastra/entry.js`): `CONVEX_URL` selects `ConvexStore`, absent it falls back to LibSQL, and every report records `vendor_store_kind` so no future run is ambiguous about which was measured. **The Convex configuration is unrun** -- it needs a Convex deployment and `CONVEX_ADMIN_KEY`, and `npx convex dev` requires an interactive browser login.

Until that runs, the defensible statement is narrower than Y.1's heading: *with each harness on its default local store, none resumes natively once that store is deleted.* Whether Mastra-on-Convex satisfies STATE-1a natively is **open**. The Deep Agents silent-resume finding is unaffected -- it is about an absent checkpoint, not about which backend held it.

## Y.2c Mastra-on-Convex re-tested — RESOLVED to a genuine native pass

**2026-08-25, with real Kimi K2.6, a real Convex deployment, and a real `SIGKILL`.**
Y.2b left this open. It is now resolved, after two false starts that are recorded rather
than silently corrected.

**False start 1.** My first "Convex passes" run reported `harness_native_resume: succeeded`
-- but `crash-child.js` and `resume.js` still hardcoded `LibSQLStore`, so `CONVEX_URL` was
never read at all. The pass was LibSQL wearing a Convex label. Fixed by routing both files
through the same `makeStore()` used in `entry.js`.

**False start 2.** With the store genuinely pluggable, `model_calls_after_resume: 0` was
still not measured: it came from `mockCallsSinceReset()`, which returns 0 unconditionally
when `DIGEST_REAL_MODEL=1` (no mock server runs). A `fetch`-level counter
(`evals/model-call-counter.js`) replaced it, validated against a case that provably calls
the model (`model_calls: 1`). `resume_path` was also asserted too cheaply -- it branched on
whether `approveToolCallGenerate()` threw, not on whether a send actually resulted. Fixed
to require `harness_produced_send` (a real persisted send record) before calling anything
"native".

**Then the finding briefly went the other way.** A first attempt to directly verify
`ConvexStore` traffic (proxying the store object) showed zero snapshot writes, because
`getStore("workflows")` returns a sub-store whose methods a shallow Proxy on the top-level
object never sees -- an artifact of the probe, not evidence Mastra writes nothing.
Re-instrumented at the network layer instead (wrapping `fetch` to the `*.convex.cloud`
host) and got the real trace:
...
**Decisive proof, independent of any Mastra code.** A run was suspended and its process
exited without ever resuming (so nothing was cleaned up). Minutes later, a plain Python
script -- raw HTTP, no `@mastra/convex`, no JS runtime in common with the original process
-- called the deployed `mastra/storage:handle` function directly and read the row back:
...
That is unrelated-process, unrelated-language confirmation that the suspended workflow
state genuinely lives in Convex, keyed by `run_id`, independent of any Node process.

(One side note that cost time and is worth recording: `npx convex data <table>` reported
"no documents" throughout, including for a row just proven present by direct query. The
CLI's table browser appears to require the table be declared with a matching name in
`convex/schema.ts` -- ours declares `mastra_workflow_snapshots` (plural, per the package's
own bundled reference), while the runtime writes to `TABLE_WORKFLOW_SNAPSHOT` =
`mastra_workflow_snapshot` (singular). The mutation handler has no `list`/`scan`/`getAll`
op, so enumeration isn't possible without the CLI recognizing the table; keyed `load` is
the only way this was actually verified. This is a docs/schema-naming mismatch worth fixing
in `convex/schema.ts` -- and a trap for anyone trusting the CLI's browser as ground truth.)

**With the instrumentation now trustworthy, the real cross-process test:**
...
args_byte_identical       True
tool_selection_events     1
model_calls_after_resume  0     (counted from real outbound HTTP, not the mock)
slack_posts               1
harness_native_resume     succeeded
...
**Verdict: Mastra-on-Convex satisfies STATE-1a natively.** A real `SIGKILL` across real OS
process boundaries, a real Kimi K2.6 model, and a real Convex deployment holding the only
copy of the suspended state -- resume works, args are byte-identical, the tool is not
re-selected, and the model is not re-invoked. This is the strongest possible reading of
"native": not a local file that happened to survive, but state that was independently
confirmed to exist nowhere except a remote database, read back by a process that did not
create it.

This revises the Y.1 headline. **"Zero of three harnesses resume natively once their own
state is gone" no longer holds for Mastra when Mastra is configured on `ConvexStore`.** It
still holds for Mastra's default LibSQL configuration (Y.1's original evidence), for Deep
Agents' default checkpointer, and for Flue -- none of those were re-tested against a vendor
store that lands in our own schema, because none of them ship one. Mastra is the only leg
where the STATE-1a-compliant configuration was directly testable, and it passes.

## Y.3 Caveats on this probe

- It deletes the vendor **file**, which is the closest local analogue to STATE-1a's intent. It is not a test against a remote vendor store (Convex, Turso, a hosted checkpointer), where "deleted" would mean something different. See Y.2b -- this is the probe's most consequential limitation, not a footnote.
